...
class Assembler:
    def __init__(self,file_path):
        ...
        read the file, ignore the blank and comment lines
        @parm file_path(str): the .asm to be translated into binary code 
        """
        self.codes = []
        self.binary_codes = []
        ...
        #处理读取的字符串 1、去除首尾空格 2、删除空格行及注释行 3、去除代码行的注释
    ...
    def deci2bin(self,decimal):
        ...
        """
        binary = bin(int(decimal))[2:] #bin(x) is like "0b010101"
    ...
    def A_command(self, command):
        ...
        node that xxx can be a symbol or a decimal
        ...
        if command.isdecimal():
            ...
            self.allo_address = self.allo_address + 1
            return binary
        else :
    ...
    def C_command(self, command):
        """
        generate binary codes for C command which like dest=co,p;jmp
